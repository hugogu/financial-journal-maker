...
### User Story 3 - 共享业务信息管理 (Priority: P3)

用户可以定义和维护共享的业务背景信息（如公司主体、支付渠道、结算周期、币种、税率等），这些信息在每次AI分析时自动作为上下文提供，避免用户每次都需要重复描述基础业务信息。

**Why this priority**: 共享业务上下文是提高AI分析准确性和用户效率的关键，减少重复输入，确保分析结果的一致性。

**Independent Test**: 用户配置业务背景信息后，新建场景分析时AI自动引用这些信息，无需用户再次说明。

**Acceptance Scenarios**:

1. **Given** 用户首次使用平台, **When** 用户在设置中配置"公司使用人民币结算，主要支付渠道为支付宝、微信、银行卡", **Then** 系统保存这些信息作为全局业务上下文
2. **Given** 已配置业务背景信息, **When** 用户输入新场景"用户充值", **Then** AI分析时自动结合已配置的支付渠道和币种信息生成更精准的科目设计
3. **Given** 用户修改共享业务信息, **When** 修改"新增VISA支付渠道", **Then** 系统提示哪些现有设计可能需要更新以支持新渠道
4. **Given** 用户查看AI分析结果, **When** 用户想了解AI使用了哪些背景信息, **Then** 系统展示本次分析引用的共享业务信息列表

---

### User Story 4 - 现有科目导入 (Priority: P4)
...
### User Story 5 - 分层设计管理 (Priority: P5)
...
### User Story 6 - Numscript导出与验证 (Priority: P6)
...
### User Story 7 - LLM模型配置 (Priority: P7)
...
- **FR-016**: 系统MUST支持定义和维护共享业务背景信息（公司主体、支付渠道、币种、结算周期等）
- **FR-017**: 系统MUST在每次AI分析时自动将共享业务信息作为上下文提供给AI
- **FR-018**: 系统MUST在分析结果中展示本次引用的共享业务信息
- **FR-019**: 系统MUST在共享业务信息变更时提示可能受影响的现有设计
...
- **业务上下文 (Business Context)**: 全局共享的业务背景信息，包含公司主体、支付渠道列表、支持币种、结算周期、税率配置等，作为AI分析的默认输入
